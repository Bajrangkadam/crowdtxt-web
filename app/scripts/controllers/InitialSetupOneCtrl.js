...
(function () {
  ...
  var module = angular.module(
    'ct.controllers.initialSetupOne',
    []
  );

  module.controller('InitialSetupOneCtrl',
    ['$rootScope', '$scope', '$location', '$http', '$timeout', '$upload', '$cookieStore',


      function InitialSetupOneCtrl($rootScope, $scope, $location, $http, $timeout, $upload, $cookieStore) {
        ...
        //alert("help");
        console.log('InitialSetupOneCtrl module is called..');
        ...
        var firstnamess = window.localStorage['companyNamefirstnameTodisplay'];
        var lastnamess = window.localStorage['companyNamelastnameTodisplay'];
        var emailss = window.localStorage['emailToDisplay'];
        // var phoness = window.localStorage['companyNamephoneTodisplay'];

        // $scope.first = firstnamess;
        // $scope.last = lastnamess;
        $scope.email = emailss;
        // $scope.mobileNo = phoness;
        ...
        function refresh() {
          $http({
            ...
            // url: 'api/v1/company/setup/'+ id
          }).success(function (data, status) {

            data = data.data.data;
            console.log("hjkhjhhhh", data);

            $scope.companyName = data.companyname;
            $scope.email = data.email;
            $scope.first = data.firstname;
            $scope.last = data.lastname;
            $scope.mobileNo = data.phone;
            $scope.isMailOtpVerified = data.isMailOtpVerified;


            if ($scope.isMailOtpVerified == "Y") {
              $scope.hideEmailVerificationBtn = true;
              $scope.disableEmailId = true;
              $scope.nextBtnDisable = false;
            }

            if ($scope.isMailOtpVerified == "N" || $scope.isMailOtpVerified == null) {
              $scope.hideEmailVerificationBtn = false;
              $scope.disableEmailId = false;
        ...
        $scope.verifyEmailIdBtn = function () {
          if ($scope.myform.$valid) {
            ...
            $scope.dataJsonEmail = {
              ...
              "lastName": $scope.last,
              "emailId": $scope.email,
              "receiverPhoneNo": $scope.mobileNo
            }
            console.log($scope.dataJsonEmail);
            $http({
              method: 'POST',
              url: 'api/v1/email/Otp',
              data: $scope.dataJsonEmail,
              headers: {
                "Content-Type": 'application/json'
              }
              ...
              $scope.hideEmailVerification = true;
              $scope.hideEmailVerificationBtn = true;
              $scope.disableEmailId = true;

              $(".showAlertOnSuccessEmailVerification").show(); // To show Toast Notification when clicked on delete..
              window.setTimeout(function () {
                $(".showAlertOnSuccessEmailVerification").hide();
              }, 8000)
        ...
        $scope.hideEmailVerification = false;
        $scope.counterss = 0;
        $scope.emailShowNextBtn = function (inc) {
          $scope.counterss += inc;
          console.log("Btn Count", $scope.counterss);

          if ($scope.counterss == 3) {
            ...
            $(".showAlertOnEmailReentering").show(); // To show Toast Notification when clicked on delete..
            window.setTimeout(function () {
              $(".showAlertOnEmailReentering").hide();
            }, 8000)
          ...
          if ($scope.myform12.$valid) {
            ...
            $http({
              method: 'POST',
              url: 'api/v1/verify/mailotp/' + $scope.emailOtpCode
              ...
              $(".showAlertOnSuccessEmailOtpCode").show(); // To show Toast Notification when clicked on delete..
              window.setTimeout(function () {
                $(".showAlertOnSuccessEmailOtpCode").hide();
              }, 8000)
              ...
              $(".showAlertOnErrorEmailOtpCode").show(); // To show Toast Notification when clicked on delete..
              window.setTimeout(function () {
                $(".showAlertOnErrorEmailOtpCode").hide();
              }, 8000)
        ...
        //                                       console.log("$scope.mobileNo",$scope.mobileNo);
